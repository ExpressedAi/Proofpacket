...
ALLOW_AXIOMS = {
    ...
    # Navier-Stokes structural axioms (from standard PDE theory)
    "paraproduct_low_high_L2": True,
    "bernstein_Linf_of_L2": True,
    "grad_shell_L2": True,
    "paraproduct_high_low_L2": True,
    "bernstein_shell_Linf": True,
    "commutator_bound": True,
    "resonant_far_bound": True,
    "shell_absorb_low_to_dissipation": True,
    "shell_absorb_high_to_dissipation": True,
    "shell_absorb_far_to_dissipation": True,
...
def scan(filepath: pathlib.Path):
    ...
    # 3) any 'admit'
    for m in re.finditer(r'\badmit\b', txt):
        line = txt.count("\n", 0, m.start()) + 1
        hits.append({"file": str(filepath), "line": line, "kind": "admit"})
...
def main(root):
    ...
    if not result["ok"]:
        ...
        print("\n✅ Formal gate PASSED: No `sorry`, `admit`, or unauthorized axioms")
        sys.exit(0)

if __name__ == "__main__":
    main(sys.argv[1] if len(sys.argv) > 1 else "proofs/lean")
